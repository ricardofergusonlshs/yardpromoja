...
import styles from "./YardPromoSearchBox.module.css";
...
const filters = [
  ...
  {
    label: "Businesses",
    href: "/browse?category=businesses",
    icon: StoreIcon,
  },
  {
    label: "Deals",
    href: "/browse?category=deals",
    icon: TagIcon,
  },
  {
    label: "Weekend",
...
export default function YardPromoSearchBox({
  defaultValue = "",
  action = "/browse",
  className = "",
}) {
  ...
  const searchHref = useMemo(() => {
    ...
    return trimmed ? `${action}?q=${encodeURIComponent(trimmed)}` : action;
  }, [action, query]);

  return (
    <section className={`${styles.searchShell} ${className}`} aria-label="Search YardPromoJa">
      <form
      ...
      >
        ...
        <Link href={searchHref} className={styles.searchButton}>
          <SearchIcon />
          <span>Search</span>
        </Link>
      ...
      <nav className={styles.filterRow} aria-label="Popular promo filters">
        {filters.map((filter) => {
          const Icon = filter.icon;
          return (
            <Link key={filter.label} href={filter.href} className={styles.filterPill}>
              <Icon />
...
function CalendarIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
      ...
      <path d="M8 13h2.2M13.8 13H16M8 16.5h2.2M13.8 16.5H16" stroke="currentColor" strokeWidth="2.1" strokeLinecap="round" />
    </svg>
  );
}
...
function UtensilsIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
      <path d="M7 3v8M10 3v8M5 3v6.3c0 1.8 1.6 3.2 3.5 3.2S12 11.1 12 9.3V3" stroke="currentColor" strokeWidth="2.1" strokeLinecap="round" />
      <path d="M8.5 12.5V21" stroke="currentColor" strokeWidth="2.1" strokeLinecap="round" />
      <path d="M17 3c1.7 1.4 2.4 3.5 2.4 6.2v1.1H17V21" stroke="currentColor" strokeWidth="2.1" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
function ToolsIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
      ...
      <path d="M4.5 6.2 7.8 9.5M6.2 4.5 9.5 7.8" stroke="currentColor" strokeWidth="2.1" strokeLinecap="round" />
    </svg>
  );
}
...
function StoreIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
      <path d="M5 10.5V20h14v-9.5" stroke="currentColor" strokeWidth="2.1" strokeLinejoin="round" />
      <path d="M4 10.5h16l-1.6-6.1H5.6L4 10.5Z" stroke="currentColor" strokeWidth="2.1" strokeLinejoin="round" />
      <path d="M8 20v-5h8v5M8 10.5v2.1M12 10.5v2.1M16 10.5v2.1" stroke="currentColor" strokeWidth="2.1" strokeLinecap="round" />
    </svg>
  );
}
