...
export function activeAds(list = sampleAds) {
  return list.filter((ad) => String(ad.status || '').toLowerCase() === 'active' || String(ad.status || '').toLowerCase() === 'approved');
}
...
export function cleanAd(ad, fallbackIndex = 0) {
  const fallback = sampleAds[fallbackIndex % sampleAds.length] || sampleAds[0];
  return {
    ...fallback,
    ...ad,
    slug: ad?.slug || String(ad?.id || fallback.slug),
    poster_image_url:
      ad?.poster_image_url ||
      ad?.image_url ||
      ad?.flyer_url ||
      ad?.cover_image ||
      fallback.poster_image_url,
  };
}

export function getAdBySlug(slug, list = sampleAds) {
  return list.find((ad) => ad.slug === slug) || sampleAds.find((ad) => ad.slug === slug) || sampleAds[0];
}

export function relatedAds(slug, limit = 3) {
  return activeAds(sampleAds).filter((ad) => ad.slug !== slug).slice(0, limit);
}
